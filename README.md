...
### 实时翻译（双语字幕）
- 开启后字幕窗口**底部双行显示**：一行原文、一行译文，**分别出字、互不干扰**（翻译后台异步，不阻塞识别）
- 只翻**定稿句**（避免中间结果频繁重译抖动），译文按句末标点自动分行
- 四种翻译引擎可切换：
  - **微软 Azure Translator**（推荐）：官方免费层 F0，每小时 200 万字符，延迟 150–300ms，稳定性最佳
  - **谷歌翻译**：免 key 备选，走免费非官方端点（高频易限流）
  - **本地 LibreTranslate** / **NLLB-200**：完全离线，复用 WSL 起服务模式，隐私优先
- Azure key 存系统保险箱（不进配置文件）；译文字号/颜色可自定义
- 详见 [实时翻译](#实时翻译)
...
- 设置对话框多标签页（左侧导航）：
  ...
  - **外观**：主题、颜色、字体、窗口尺寸
  - **行为**：滚动、字幕文本上限等
  - **翻译**：双语字幕（原文+译文双行），支持 Azure/Google/本地翻译引擎切换（详见 [实时翻译](#实时翻译)）
  - **皮肤**：桌宠贴图
...
开启翻译后，字幕窗口底部会多出一行译文，与原文上下排列、**各自出字互不干扰**——识别出的原文实时显示，译文在后台异步翻译完成后独立出现，互不阻塞。

### 工作方式
- **只翻定稿句**：识别引擎输出一个完整句子（句末标点 / 引擎边界）后才送翻译，避免中间结果逐字重译造成的抖动和配额浪费
- **原文译文双流解耦**：翻译在独立后台线程池执行，翻译慢不会拖慢识别；翻译失败只影响译文行，原文字幕照常工作
- **去重缓存**：同一句话不重复翻译（部分引擎会累计回调历史句，内部自动去重）

### 四种翻译引擎

| 引擎 | 需要 key | 免费 | 延迟 | 适合 |
|---|---|---|---|---|
| **微软 Azure Translator**（推荐） | 是（存保险箱） | F0 每小时 200 万字符 | 150–300ms | 日常首选，稳定低延迟 |
| 谷歌翻译 | 否 | 完全免费（易限流） | 中 | 不想配 key 的备选 |
| 本地 LibreTranslate | 否 | 离线免费 | 几百 ms~1s | 隐私优先 / 断网兜底 |
| 本地 NLLB-200 | 否 | 离线免费 | 300ms~2s | 质量最高 / 冷门语对 |
...
2. 选引擎、源语言（自动检测）、目标语言
3. 按引擎填配置：
   - **Azure**：填 Key + Region（存系统保险箱，与阿里云凭证同等安全）。申请：[Azure 门户](https://portal.azure.com/) → 创建「翻译器」资源 → 取 Key1 + 区域，选 F0 免费层
   - **Google**：无需配置（免 key）
   - **LibreTranslate / NLLB**：填本地服务 host:port（见下方启动方式）
4. 点「**测试连接**」验证，再「应用」

### 本地翻译服务（LibreTranslate / NLLB-200）

复用与 Fun-ASR-Nano 流式相同的「WSL 起服务 + Windows localhost 直连」模式：

**LibreTranslate**（开箱即用 HTTP API，~40 语言）：
```bash
# WSL 里一行起（模型自动下载，--load-only 只下指定语言包省磁盘）
docker run -p 5000:5000 libretranslate/libretranslate --load-only en,zh,ja
```

**NLLB-200**（Meta 200 语言，质量/覆盖最强）：
```bash
# WSL 里安装 + 启动（默认模型 facebook/nllb-200-distilled-600M，约 1.2GB，CPU 可跑）
pip install git+https://github.com/thammegowda/nllb-serve
nllb-serve --port 6060
```

> 本地引擎在 CPU 上每句几百毫秒到 2 秒，比云端稍慢；显存充足时 NLLB 可换更大模型提升质量。

### 译文样式
- **译文字号**：原文字号 × 倍率（默认 0.85，让原文更突出）
- **译文颜色**：默认跟随主题文本色，可自定义（如让译文偏淡灰）
- 译文行也支持句末标点自动分行，与原文规则独立
...
- **顶部硬件摘要 + 🔄 重新检测按钮**：显示当前 CPU 核数 / 内存 / GPU。装完依赖后点「重新检测」，引擎状态和硬件信息会立即刷新（无需重启程序）。
- **每个本地引擎一张卡片**：
...
依赖探测用 `find_spec`（开销极低）。**装完依赖后点「🔄 重新检测」**，就能立即看到对应引擎变成 `✅ 已就绪`——无需重启程序。
